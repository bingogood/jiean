	...
	fxuiTab:function(opt){
		...
		return this.each(function() {
			...
			var t = $(this),//t就是$(this)，因为Jquery里$(this)很多，我们可以把这个t选存起来，后面好多地方可以用。
			    ...
			    tit = o.tit || t.find('.fxui-tab-tit,.item-photo'),  //标签点击的对象。
				nav = o.nav || t.find('.tabs-content,.box-service-txt'),  //内容显示的对象。
			...
			tit.bind(evt,function(){ //通过bind 传这个evt,事件就可以变动了，不一定只是click事件了。
				$(this).addClass('active ').siblings(tit).removeClass('active ');//处理标签头：加当前的Class，样式通过css改变
				nav.eq(tit.index($(this))).show().siblings('.tabs-content,.box-service-txt').hide();//内容块显示。
			}).hover(function(){
